	...
	repeat
		...
		if not esfin then begin
			if juego.numbarcos = 0 then begin
				...
				else if barco.estado = intacto then begin
					juego.numintactos := juego.numintactos + 1;
				end
				else begin
					juego.numnohundidos := juego.numnohundidos + 1;
				end;	
	...
	for fila := 1 to juego.numfilas do begin
		...
		for column := 1 to juego.numcolumnas do begin
			if (juego.numbarcos <> 0) and casillacoincidenteproa(juego,columna,fila) then begin
				if (columna = 'A') and (fila = 1) then begin
					write('E ');
				end
				else begin
					write('P ');
...
begin
	...
	writeln(juego.numintactos, ' Intactos');
	writeln(juego.numnohundidos, ' No Hundidos');
